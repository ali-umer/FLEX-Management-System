...
public partial class Aalot : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string query3 = "select  distinct section from studies where courseID = @course";
        ...
        if (!IsPostBack)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                Session["facultyalot"] = DropDownList1.SelectedValue;
                connection.Close();
                ...
                Session["coursealot"]= DropDownList2.SelectedValue;
                connection.Close();
                ...
                using (SqlCommand command = new SqlCommand(query3, connection))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            string name = reader["section"].ToString();
                ...
                Session["sectionalot"] = DropDownList3.SelectedValue;
                connection.Close();
        ...
        {
            previouscourse = (string)Session["coursealot"];
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        ...
        string faculty_ID = (string)Session["facultyalot"];
        string section_ID = (string)Session["sectionalot"];
        string course_ID = (string)Session["coursealot"];
        string query1 = "select count(*) from teaches where facultyid = @facultyID";
        int count = 0;
        ...
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            ...
            command1.Parameters.AddWithValue("@facultyid", faculty_ID);
            count = Convert.ToInt32(command1.ExecuteScalar().ToString());
            if(count<3)
            {
                string query2 = "INSERT INTO teaches (facultyID, section, courseID) VALUES ('" + faculty_ID + "','" + section_ID + "','" + course_ID + "')";
