...
public partial class setweightages : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string query1 = "select distinct C.name from teaches T join courses C on T.courseID = C.ID where facultyid =@username";
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        ...
        if (quizMarks + assignmentMarks  + sessionalMarks + projectMarks + finalMarks == 100)
        {
            string query = "UPDATE courses SET assignmentweightage = @assignmentMarks, quizweightage= @quizMarks ,sessionalsweightage = @sessionalMarks, finalsweightage =@finalMarks, projectweightage = @projectMarks WHERE name=@coursename;";
