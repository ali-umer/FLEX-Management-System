...
public partial class _Default : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string query1 = "Select RollNum from students where RollNum = @username";
        string query2 = "select degree from students where RollNum = @username";
        string query3 = "Select batch from students where RollNum = @username";
        string query4 = "Select regNo from students where RollNum = @username";
        string query5 = "select campus from students S join users U on U.username = S.RollNum AND S.RollNum = @username";
        string query6 = "select status from students where RollNum = @username";
        string query7 = "select name from students S join users U on U.username = S.RollNum AND S.RollNum = @username";
        string query8 = "select DOB from students S join users U on U.username = S.RollNum AND S.RollNum = @username";
        string query9 = "select bloodGroup from students S join users U on U.username = S.RollNum AND S.RollNum = @username";
        string query10 = "select gender from students S join users U on U.username = S.RollNum AND S.RollNum = @username";
        string query11 = "select CNIC from students S join users U on U.username = S.RollNum AND S.RollNum = @username";
        string query12 = "select nationality from students S join users U on U.username = S.RollNum AND S.RollNum = @username";
        string query13 = "select email from students S join users U on U.username = S.RollNum AND S.RollNum = @username";
        string query14 = "select contact from students S join users U on U.username = S.RollNum AND S.RollNum = @username";
        string query15 = "select address from students where RollNum = @username";
        string query16 = "select city from students where RollNum = @username";
        string query17 = "select country from students where RollNum = @username";
        ...
        if (!IsPostBack)
        {
            string query_a = "SELECT courseID, total, presents, (presents * 100.0 / total) AS percentage FROM ( SELECT courseID, COUNT(date) AS total, SUM(CASE WHEN status = 'P' THEN 1 ELSE 0 END) AS presents FROM attendance    WHERE studentID = @username   GROUP BY courseID) AS subquery";
            string query_b = "SELECT C.semester, ROUND(SUM(CASE G.grade WHEN 'A+' THEN 4.0 WHEN 'A' THEN 4.0 WHEN 'A-' THEN 3.7 WHEN 'B+' THEN 3.3 WHEN 'B' THEN 3.0 WHEN 'B-' THEN 2.7 WHEN 'C+' THEN 2.3 WHEN 'C' THEN 2.0 WHEN 'C-' THEN 1.7 WHEN 'D+' THEN 1.3 WHEN 'D' THEN 1.0 ELSE 0 END * C.credithours) / SUM(C.credithours), 2) AS gpa FROM students JOIN grades G ON G.studentID = students.RollNum JOIN courses C ON C.ID = G.courseID WHERE students.RollNum = @username GROUP BY students.RollNum, C.semester;\r\n";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                Chart1.Series["Series1"].Points.DataBindXY(dt_a.Rows, "courseID", dt_a.Rows, "percentage");
