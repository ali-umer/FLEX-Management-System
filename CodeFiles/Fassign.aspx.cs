...
public partial class Fassign : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string course = (string)Session["coursegrades"];
        string query1 = "select distinct S.studentid,U.name from studies S join users U on U.username = S.studentid join teaches T on T.courseID = S.courseid where S.courseid = @course and S.section = @section AND T.facultyID = @username group by S.studentid,U.name";
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        ...
        foreach (GridViewRow row in GridView1.Rows)
        {
            ...
            string query = "INSERT INTO grades (studentID, courseID,grade) VALUES ('" + rollNumber + "', '" + course + "', '" + grade + "')";
    ...
    protected void Button3_Click(object sender, EventArgs e)
    {
        ...
        SqlCommand command = new SqlCommand("select distinct studentID,name,grade from grades join teaches on teaches.courseID = grades.courseID join users on users.username = grades.studentID where teaches.facultyid = @username AND grades.courseID = @course", connection);
        ...
        using (MemoryStream memoryStream = new MemoryStream())
        {
            ...
            {
                cell = new PdfPCell(new Phrase(row["studentID"].ToString()));
                cell.HorizontalAlignment = Element.ALIGN_CENTER;
                table.AddCell(cell);
