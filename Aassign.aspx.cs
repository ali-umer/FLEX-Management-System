...
public partial class Aassign : System.Web.UI.Page
{
    ...
    protected void Button2_Click(object sender, EventArgs e)
    {
        ...
        string connectionString = "Data Source=ALI\\SQLEXPRESS;Initial Catalog=flex;Integrated Security=True";
        string query1 = "select count(*) from studies where studentID = @id";
        int totalcourses = 0;
        bool flag = false;
        string query2 = "select prereqID from prereq where ID = @course";
        string prereq = "";
        ...
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            ...
            else
            {
                ...
                if (result != DBNull.Value && result != null)
                {
                    ...
                    string query3 = "select grade from grades where courseID = @prereq AND studentID = @id AND grade != 'F'";
                ...
                if (flag)
                {
                    string query = "INSERT INTO studies (studentid, courseid) VALUES ('" + id + "', '" + course + "')";
